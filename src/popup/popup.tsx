...
function Popup() {
  ...
  const handleCheckedBoxChange = (id: string) => {
    const updatedTodos = todo.map((todo) =>
      todo.id === id
        ? { ...todo, checked: !todo.checked, status: !todo.status }
        : todo
    );
    ...
    if (import.meta.env.VITE_IS_EXTENSION === "true") {
      chrome.storage.local.get(["items"], (result) => {
        ...
        const updateChecked = getList.map((its) =>
          its.id === id
            ? { ...its, checked: !its.checked, status: !its.status }
            : its
        );
        ...
        chrome.storage.local.get(["color"],(result) => {
            const updateCheck = result.color === "active"
        ? updateChecked.filter((item) => !item.status)
        : result.color === "completed"
        ? updateChecked.filter((item) => item.status)
        : updateChecked
      chrome.storage.local.set({ items1: updateCheck }, () => {
        setActiveColor(result.color)
  ...
  const deleteItemHandler = (id: string) => {
    ...
    if (import.meta.env.VITE_IS_EXTENSION === "true") {
      ...
      chrome.storage.local.get(["items1"], (result) => {
        const getList: User[] = Array.isArray(result.items1)
          ? result.items1
          : [];
        const updateChecked = getList.filter((its) => its.id !== id);
  ...
  const getActiveHandler = () => {
    ...
    if (import.meta.env.VITE_IS_EXTENSION === "true") {
        chrome.storage.local.set({color:'active'},() => {
           chrome.storage.local.get(["items"], (result) => {
  ...
  const getAllHandler = () => {
    setActiveColor("all");
    console.log("this code run second")
    
    setDisplayedTodos(todo);
    if (import.meta.env.VITE_IS_EXTENSION === "true") {
        chrome.storage.local.set({color:'all'},() => {
          chrome.storage.local.get(["items"], (result) => {
        const todo: User[] = Array.isArray(result.items) ? result.items : [];
  ...
  const completedHandler = () => {
    ...
    setDisplayedTodos(getActive);
    if (import.meta.env.VITE_IS_EXTENSION === "true") {
      chrome.storage.local.set({color:'completed'},() => {
        chrome.storage.local.get(["items"], (result) => {
  ...
  return (
    <div className="w-[375px] bg-very-light-gray min-h-[700px] h-screen overflow-y-scroll">
      <div className="bg-[url('/background.jpg')] bg-cover bg-center h-40 relative pt-8 px-4">
        ...
        <div>
          ...
          <div className="bg-white rounded-lg p-4 -translate-y-4 shadow-lg shadow-black/10">
            <ul className="space-y-4 ">
              {displayedTodos.length === 0 && (
                <p className="font-Josefin text-gray-800 font-medium flex justify-center text-base">
                  No tasks for now
                </p>
              )}
              {displayedTodos.slice().reverse().map((item) => (
                <li
            ...
            <div className="flex font-Josefin justify-between items-center  pt-4  text-gray-600 font-semibold text-sm pb-14">
              <p>{`${displayedTodos.length} items left`}</p>
              <button
                className="bg-white hover:bg-white hover:text-gray-700"
                onClick={getClearHandler}
              >
                {" "}
                Clear Completed
              </button>
          ...
          <div className="bg-white rounded-lg  fixed bottom-4 left-0 right-0 z-50" style={{ margin: '0 auto', width: '300px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.3), 0 4px 6px -2px rgba(0, 0, 0, 0.2)', }}>
            <ul className="flex font-Josefin gap-4 items-center justify-center px-12 py-4  text-gray-600 font-semibold text-sm cursor-pointer">
              <li
                className={`select-none ${
                  activeColor === "all" && "text-blue-600 hover:text-blue-800"
                }`}
              ...
              <li
                className={`hover:text-gray-700 select-none ${
                  activeColor === "active" &&
                  "text-blue-600 hover:text-blue-800"
                }`}
                onClick={getActiveHandler}
              >
                Active
              </li>
              <li
